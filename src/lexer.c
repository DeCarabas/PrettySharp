...
struct Token scan_interpolated_string_literal(bool verbatim) {
  ...
  // through multiple calls to the lexer. This has advantages and disadvantages,
  // but an advantage is that I don't have to keep a stack inside the lexer, so
  // it's ever so slightly simpler.
